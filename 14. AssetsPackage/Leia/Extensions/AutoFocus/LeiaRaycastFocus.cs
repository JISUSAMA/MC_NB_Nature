...
namespace LeiaUnity
{
    ...
    [HelpURL("https://support.leiainc.com/sdk/unity-plugin/leiasr-tm-unity-plugin-guide/extensions/auto-focus/leiaraycastfocus")]
    public class LeiaRaycastFocus : LeiaFocus
    {
        ...
        private void CalculateTargetConvergenceAndBaseline()
        {
            ...
            if (hits > 0)
            {
                ...
                this.SetTargetFocalDistance(newTargetConvergenceDistance);

                float nearPlaneBestBaseline = LeiaDisplayUtils.GetRecommendedDepthFactorWithNearPlane(
                    leiaDisplay,
                    closestDistance
                );

                float farPlaneBestBaseline = LeiaDisplayUtils.GetRecommendedDepthFactorWithFarPlane(
                    leiaDisplay,
                    furthestDistance
                );

                float targetBaseline = Mathf.Min(nearPlaneBestBaseline, farPlaneBestBaseline);
                this.SetTargetDepthFactor(targetBaseline);
            }
        }
        ...
        void SampleSpherecast(Vector3 startPoint, Vector3 direction, float radius, int sampleIndex)
        {
            ...
            else
            {
                if (showDebugRaycasts)
                {
                    Debug.DrawRay(startPoint, direction * targetFocalDistance * 2f, Color.red);
                    Vector2 c0 = cameraNearPlaneRaysOrigins[sampleIndex];
                    Vector2 cbottom = c0 + new Vector2(0, -crossScale);
                    Vector2 ctop = c0 + new Vector2(0, crossScale);
                    Vector2 cleft = c0 + new Vector2(-crossScale, 0);
