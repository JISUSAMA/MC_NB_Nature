...
namespace LeiaUnity
{
    ...
    public class LeiaDisplay : MonoBehaviour
    {
        ...
        public enum DisplayMode {_3D, _2D};
        [SerializeField] private DisplayMode _displayMode;
        public DisplayMode displaymode { get { return _displayMode; } set { this.Set3DMode(value == DisplayMode._3D); } }
        ...
        [SerializeField] private float fovFactor = 1.0f;
        ...
        public float FocalDistance
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    LogUtil.Log(LogLevel.Error, "Error: attempted to set focal distance in Display Driven mode");
                }
            }
        }
        ...
        void ValidateFocalDistance()
        {
            if (mode == ControlMode.CameraDriven)
            {
                if (focalDistance != FocalDistance && focalDistance != 0)
                {
        ...
        public void SetFocalDistance(float value)
        {
            focalDistance = value;
        }
        ...
        /// </summary>
        /// <param name="screenPoint">A screenPosition</param>
        /// <returns>A Vector3 representing screenPoint in world space coordinates</returns>
        public Vector3 ScreenToWorldPoint(Vector3 screenPoint)
        {
            return LeiaDisplayUtils.ScreenToWorldPoint(this, screenPoint);
            
            /*Vector3 worldPoint;
            Vector3 localPoint = new Vector3(
                screenPoint.x / Screen.width * VirtualWidth - VirtualWidth / 2f,
                screenPoint.y / Screen.height * VirtualHeight - VirtualHeight / 2f,
                0
            );
            worldPoint = transform.position + transform.rotation * localPoint;
        ...
        {
            return LeiaDisplayUtils.WorldToScreenPoint(this, worldPosition);
        }

        public Ray ScreenPointToRay(Vector3 screenPosition)
        {
            return LeiaDisplayUtils.ScreenPointToRay(this, screenPosition);
        }
        ...
        public void Update()
        {
            ...
            {
                UIEventCamera.fieldOfView = ViewersHead.headcamera.fieldOfView;
                UIEventCamera.transform.position = HeadPosition;
                Matrix4x4 p = GetProjectionMatrixForCamera(UIEventCamera, Vector3.zero, false);
                UIEventCamera.projectionMatrix = p;
            }
            else
            {
                LeiaEventCamera leiaEventCamera = GetComponentInChildren<LeiaEventCamera>();
                if (leiaEventCamera != null)
                {
                    UIEventCamera = leiaEventCamera.GetComponent<Camera>();
                    if (leiaEventCamera != null)
                    {
                        UIEventCamera = leiaEventCamera.GetComponent<Camera>();
                    }
                    ...
                    {
                        LogUtil.Log(LogLevel.Error, "UIEventCamera is null");
                    }
                }
            }
...
//#endif
        ...
        public void InitLeiaDisplay(List<Vector2> ViewConfig)
        {
            ...
            GameObject UIEventCameraGO = new GameObject("UIEventCamera");
            UIEventCameraGO.transform.parent = transform;
            UIEventCamera = UIEventCameraGO.AddComponent<Camera>();
            UIEventCamera.cullingMask = 1 << LayerMask.NameToLayer("UI");
            UIEventCamera.depth = 100;
            UIEventCamera.clearFlags = CameraClearFlags.Nothing;
            UIEventCamera.cullingMask = 0;
            UIEventCamera.fieldOfView = ViewersHead.headcamera.fieldOfView;
            UIEventCamera.transform.localPosition = ViewersHead.headcamera.transform.localPosition;
            UIEventCamera.gameObject.AddComponent<LeiaEventCamera>();
        }
        ...
        public Vector3 HeadPosition
        {
            get
            {
                return (GetEyeCamera(0).transform.position + GetEyeCamera(1).transform.position) * 0.5f;
            }
        }
        ...
        public int GetViewCount()
        {
            ...
            if (ViewersHead == null || ViewersHead.ViewConfig == null)
            {
                return 1;
            }
            ...
            if(supportAllAxis)
            {
                ...
                if (currentOrientation != _previousOrientation)
                {
                        ...
                        (IsLandscape(Screen.orientation) && deviceNaturalOrientation == Leia.Orientation.Portrait))
                    {
                        ...
                        LogUtil.Log(LogLevel.Debug, $"Update Views - Landscape viewResolutionX: {viewResolutionX} , viewResolutionY: {viewResolutionY}");
                             ...
                             (IsPortrait(Screen.orientation) && deviceNaturalOrientation == Leia.Orientation.Portrait))
                    {
                        ...
                        LogUtil.Log(LogLevel.Debug, $"Update Views - Portrait viewResolutionX: {viewResolutionX} , viewResolutionY: {viewResolutionY}");
        ...
        {
            LogUtil.Log(LogLevel.Debug, "Set3DMode: " + toggle);

            if (toggle)
            {
                this._displayMode = DisplayMode._3D;
            }
            else
            {
                this._displayMode = DisplayMode._2D;
            }
            UpdateLightfieldMode();
        }

        private void UpdateLightfieldMode()
        {
            if (!Application.isPlaying)
            {
                return;
            }
            if (this.displaymode == DisplayMode._3D)
            {
                RenderTrackingDevice.Instance.DesiredLightfieldMode = LightfieldMode.On;
            }
            else
            {
                RenderTrackingDevice.Instance.DesiredLightfieldMode = LightfieldMode.Off;
            }
        }
